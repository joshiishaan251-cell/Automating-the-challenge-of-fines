...
def fill_template(template_path, output_path, data):
    ...
    try:
        ...
        print("Библиотека python-docx не установлена. Установите её: pip install python-docx")
        return False
    ...
    try:
        ...
        replacements = {
            ...
            # Запасные варианты на случай, если вы ввели в шаблон переменные без скобок
            "CASE_NUMBERS": ", ".join(data.get("CaseNumbers", [])),
            "VEHICLE_NUMBERS": ", ".join(data.get("VehicleNumbers", [])),
            "POCHTA_NUMBER": data.get("PochtaNumber", ""),
            "DATA_NUMBER": data.get("DataNumber", "")
...
def get_page_count(doc_obj):
    ...
    try:
        # docx uses app.xml for page count. This is a cached value and might be stale.
...
def convert_to_pdf(docx_path, output_dir):
    if not os.path.exists(LIBREOFFICE_PATH):
        print(f"Ошибка: LibreOffice не найден по пути {LIBREOFFICE_PATH}")
        return False
    
    try:
        print(f"Конвертация в PDF: {docx_path.name}...")
        cmd = [
            LIBREOFFICE_PATH,
            "--headless",
            "--convert-to", "pdf",
...
def generate_for_folder(folder_path, config=None):
    ...
    print(f"\n--- Генерация документов для: {folder_path.name} ---")
    ...
    print(f"  DataNumber  в базе: {data.get('DataNumber', '[НЕТ]')}")
    print(f"  PochtaNumber в базе: {data.get('PochtaNumber', '[НЕТ]')}")
    print(f"  CaseNumbers  в базе: {data.get('CaseNumbers', '[НЕТ]')}")
    ...
    # We create a temporary docx to get page count if needed, or just generate final
    output_docx_temp = folder_path / f"temp_gen.docx"
    if fill_template(template_path, output_docx_temp, data):
        # Open the generated doc to get page count from properties
        page_count_str = ""
        try:
            from docx import Document
            ...
            if page_count:
                page_count_str = f"-{page_count}л"
        except:
            pass
        
        final_base_name = f"Заявление оспаривании постановления {folder_name_clean}{page_count_str}"
        ...
        if convert_to_pdf(output_docx, folder_path):
            print(f"Успешно созданы:\n- {output_docx.name}\n- {output_pdf.name}")
        else:
            print("Создан только DOCX, ошибка конвертации в PDF.")
    else:
        print("Ошибка генерации документов.")

if __name__ == "__main__":
    if len(sys.argv) > 1:
        generate_for_folder(sys.argv[1])
    else:
        # If run without args, try current dir if it looks like a task folder
