...
# Folder pattern: Letter (Cyrillic) + 1-3 digits (e.g. А1, Б13, В123)
FOLDER_RE = re.compile(CONFIG.get('folder_pattern', r'^[А-Я]\d{1,3}$'))
...
class DuplicateDialog(tk.Toplevel):
    ...
    def __init__(self, master, uin, paths):
        ...
        self.title(f"Найден дубликат: {uin}")
        self.geometry("600x400")
        self.attributes('-topmost', True)
        
        tk.Label(self, text=f"УИН {uin} уже проиндексирован!", font=("Arial", 12, "bold"), fg="red", pady=10).pack()
        tk.Label(self, text="Этот номер встречается в следующих файлах:").pack()
        ...
        btn_copy = tk.Button(self, text="Копировать пути", command=lambda: self.copy_paths(paths))
        btn_copy.pack(pady=5)

        tk.Label(self, text="Что сделать с текущим файлом?", pady=10).pack()
        
        btn_fm = tk.Frame(self)
        btn_fm.pack(pady=10)
        
        tk.Button(btn_fm, text="Удалить в корзину и ПРОДОЛЖИТЬ", bg="#f44336", fg="white", 
                  padx=10, command=self.on_delete).pack(side=tk.LEFT, padx=10)
        tk.Button(btn_fm, text="Пропустить папку (СТОП)", bg="#9E9E9E", fg="white",
                  padx=10, command=self.on_stop).pack(side=tk.LEFT, padx=10)

    def copy_paths(self, paths):
        self.clipboard_clear()
        self.clipboard_append("\n".join(paths))
        messagebox.showinfo("Инфо", "Пути скопированы в буфер обмена")
...
class ResolutionForm(tk.Toplevel):
    ...
    def __init__(self, master, pdf_path, initial_uin, initial_date, initial_veh, is_duplicate):
        ...
        self.title(f"Обработка: {os.path.basename(pdf_path)}")
        self.state('zoomed')
        
        self._setup_ui()
        if is_duplicate:
            messagebox.showerror("Дубликат", f"УИН {initial_uin} уже есть в центральном индексе!")
            self.destroy()
    ...
    def _setup_ui(self):
        ...
        try:
            ...
            tk.Label(self.frame_viewer, text=f"Ошибка загрузки PDF: {e}", fg="white").pack()
        ...
        tk.Label(self.frame_form, text="Проверка постановления", font=("Arial", 14, "bold")).pack(pady=(0, 20))

        tk.Label(self.frame_form, text="Номер постановления (УИН):").pack(anchor="w")
        ...
        tk.Label(self.frame_form, text="Дата постановления:").pack(anchor="w")
        ...
        tk.Label(self.frame_form, text="Номер автомобиля:").pack(anchor="w")
        ...
        btn_save = tk.Button(self.frame_form, text="СОХРАНИТЬ", bg="#4CAF50", fg="white", 
                             font=("Arial", 12, "bold"), command=self.on_save)
        btn_save.pack(fill=tk.X, pady=10)

        btn_skip = tk.Button(self.frame_form, text="ПРОПУСТИТЬ", bg="#9E9E9E", fg="white", 
    ...
    def on_save(self):
        ...
        if not uin:
            messagebox.showwarning("Внимание", "УИН не может быть пустым")
            return
...
class ResolutionProcessor:
    ...
    def move_to_trash(self, file_path: Path):
        ...
        try:
            ...
            print(f"Ошибка при перемещении в корзину (PS): {e}")
            try:
                file_path.unlink()
            except Exception as e2:
                print(f"Ошибка при удалении файла: {e2}")
                return False

        # Verify file was actually removed from disk
        if file_path.exists():
            print(f"  ПРЕДУПРЕЖДЕНИЕ: Файл {file_path.name} ещё существует после операции удаления!")
            return False  # Signal failure even if PS returned OK

        print(f"Файл перемещен в корзину: {file_path.name}")
        return True
    ...
    def update_existing_report(self, uin: str, archive_path: str, filename: str):
        ...
        try:
            ...
            print("openpyxl не установлен, обновление отчета пропущено.")
            return
        ...
        if not reports:
            print("Отчет для обновления не найден (файлы UIN_Duplicates_Report_*.xlsx отсутствуют).")
            return
            
        latest_report = max(reports, key=os.path.getmtime)
        print(f"Обновление отчета: {os.path.basename(latest_report)}")
        
        try:
            wb = openpyxl.load_workbook(latest_report)
            if "Все УИН" in wb.sheetnames:
                ws = wb["Все УИН"]
                ...
                print(f"Строка добавлена в отчет и выделена красным.")
            else:
                print("Лист 'Все УИН' не найден в отчете.")
        except PermissionError:
            messagebox.showwarning("Ошибка", "Файл отчета Excel открыт! Закройте его. Запись УИН в отчет пропущена.")
            if 'tmp_path' in locals() and os.path.exists(tmp_path):
                os.remove(tmp_path)
        except Exception as e:
            print(f"Ошибка при обновлении Excel-отчета: {e}")
    ...
    def perform_ocr_on_pdf(self, pdf_path: Path) -> str:
        ...
        try:
            ...
            print(f"Выполняется OCR для: {pdf_path.name}...")
    ...
    def extract_doc_info(self, pdf_path: Path) -> Dict[str, str]:
        ...
        if self.config.get('perform_ocr', True):
            # Check against skip list from config (e.g. 'Вручено' files are not resolutions)
            ...
            if should_skip_ocr:
                print(f"  OCR пропущен (по настройкам конфига) для: {pdf_path.name}")
        ...
        if text:
            ...
            # Russian plates + English homoglyphs which Tesseract might mix up
            # Removed \\b because replacing spaces destroys word boundaries
            ...
            if veh_match:
                ...
                # Russian plates ONLY use: А В Е К М Н О Р С Т У Х
                latin_to_cyr = {
                    'A': 'А', 'B': 'В', 'C': 'С', 'E': 'Е', 'H': 'Н',
                    'K': 'К', 'M': 'М', 'O': 'О', 'P': 'Р', 'T': 'Т',
                    'X': 'Х', 'Y': 'У'
                ...
                if normalized != raw_plate:
                    print(f"  Нормализован номер авто: {raw_plate} -> {normalized}")
                info["veh"] = normalized
    ...
    def update_database_json(self, folder_path: Path, uin: str, date_str: str, veh: str):
        ...
        # NOTE: DataNumber = дата ВРУЧЕНИЯ (из файла "Вручено").
        # Дата из постановления НЕ должна перезаписывать DataNumber.
        # Она сохраняется только внутри CaseNumbers.
        
        case_entry = f"№ {uin} от {date_str}"
        ...
        try:
            ...
            print(f"База успешно обновлена: {uin}")
        except Exception as e:
    ...
    def find_active_folder(self) -> Optional[Path]:
        ...
        if not root.exists():
            print(f"Ошибка: Корень сканирования не найден: {root}")
            return None
        ...
        if not candidates:
            print(f"В папке {root} нет папок, подходящих под шаблон (например, Я1, А13...).")
            return None

        if len(candidates) == 1:
            print(f"Найдена одна папка: {candidates[0].name} — выбрана автоматически.")
            return candidates[0]

        # Terminal selection
        print("\nДоступные папки для обработки:")
        print("-" * 40)
        for i, c in enumerate(candidates, start=1):
            ...
            if db_path.exists():
                try:
                    ...
                    pass
            status = f"{done}/4 обработано" if done else "новая"
            print(f"  {i}. {c.name}  [{status}]")
        print("-" * 40)

        while True:
            try:
                raw = input(f"Введите номер папки (1-{len(candidates)}): ").strip()
                choice = int(raw)
                if 1 <= choice <= len(candidates):
                    selected = candidates[choice - 1]
                    print(f"Выбрана папка: {selected.name}\n")
                    return selected
                else:
                    print(f"  Введите число от 1 до {len(candidates)}.")
            except ValueError:
    ...
    def extract_delivery_info(self, folder_path: Path) -> Dict[str, str]:
        """Look for 'Вручено' file and extract DataNumber/PochtaNumber from its name.
        Uses os.listdir for case-insensitive matching to handle Windows encoding issues.
        """
        info = {}
        try:
            files = os.listdir(folder_path)
        except Exception as e:
            print(f"Ошибка чтения папки для поиска 'Вручено': {e}")
            return info

        for fname in files:
            if fname.lower().startswith('вручено'):
                print(f"  Найден файл доставки: {fname}")
                # Pattern: Вручено 20.02.2026 ... РПО 63097717173898
                date_match = re.search(r'Вручено\s+(\d{2}\.\d{2}\.\d{4})', fname, re.IGNORECASE)
                if not date_match:
                    # Fallback: any date in the filename
                    date_match = re.search(r'(\d{2}\.\d{2}\.\d{4})', fname)

                rpo_match = re.search(r'РПО\s*(\d+)', fname, re.IGNORECASE)

                if date_match:
                    info["DataNumber"] = date_match.group(1)
                    print(f"  Извлечена дата доставки: {info['DataNumber']}")
                if rpo_match:
                    info["PochtaNumber"] = rpo_match.group(1)
                    print(f"  Извлечён номер РПО: {info['PochtaNumber']}")
                break  # Stop after first matching file
        
        if not info:
            print("  Файл 'Вручено...' не найден в папке. DataNumber и PochtaNumber не обновлены.")
        return info

    def run(self):
        folder = self.find_active_folder()
        if not folder:
            messagebox.showinfo("Инфо", "Активная папка не выбрана или не найдена.")
            return
        ...
        # Load current state (or get default structure if it doesn't exist)
        ...
        if not db_path.exists():
            try:
                with open(db_path, 'w', encoding='utf-8') as f:
                    json.dump(db, f, ensure_ascii=False, indent=4)
                print(f"Инициализирован файл базы: {db_path}")
            except Exception as e:
        ...
        # --- Phase 1: Extract Delivery Info from "Вручено" file ---
        print("\n[Этап 1] Поиск файла доставки ('Вручено...')...")
        delivery_info = self.extract_delivery_info(folder)
        if delivery_info:
            updated = False
            if "DataNumber" in delivery_info:
                old_val = db.get("DataNumber", "Нет")
                db["DataNumber"] = delivery_info["DataNumber"]
                if old_val != delivery_info["DataNumber"]:
                    print(f"  -> DataNumber изменён: {old_val} -> {delivery_info['DataNumber']}")
                    updated = True
            if "PochtaNumber" in delivery_info:
                old_val = db.get("PochtaNumber", "Нет")
                db["PochtaNumber"] = delivery_info["PochtaNumber"]
                if old_val != delivery_info["PochtaNumber"]:
                    print(f"  -> PochtaNumber изменён: {old_val} -> {delivery_info['PochtaNumber']}")
                    updated = True
            ...
            try:
                with open(db_path, 'w', encoding='utf-8') as f:
                    json.dump(db, f, ensure_ascii=False, indent=4)
                if updated:
                    print("  База данных обновлена данными из файла 'Вручено'.")
                else:
                    print("  Данные доставки совпадают с базой, изменений нет.")
            except Exception as e:
                print(f"  Ошибка сохранения базы после обновления данных доставки: {e}")


        pdf_files = sorted(list(folder.glob("*.pdf")))

        # Load skip patterns from config — these files are not resolutions (e.g. "Вручено", "Заявление")
        skip_patterns = self.config.get('ocr_skip_patterns', [])
        ...
        for pdf in pdf_files:
            ...
            if any(fname_lower.startswith(p.lower()) for p in skip_patterns):
                print(f"Пропуск (не постановление, по конфигу): {pdf.name}")
                continue
            ...
            if match and match.group(1) in processed_uins:
                print(f"Пропуск (уже в базе): {pdf.name}")
                continue
        ...
        if not to_process:
            if len(processed_uins) >= target_count:
                messagebox.showinfo("Инфо", f"В папке {folder.name} уже обработано {target_count} постановлений.")
            else:
                messagebox.showinfo("Инфо", f"В папке {folder.name} нет новых PDF файлов для обработки.")
            # НЕ выходим! Даём возможность сгенерировать документы
        ...
        for pdf_path in to_process:
            ...
            if uin and self.is_in_central_db(uin):
                ...
                if dlg.action == 'delete':
                    if self.move_to_trash(pdf_path):
                        print(f"Файл {pdf_path.name} удален в корзину. Продолжаем.")
                        continue
                    else:
            ...
            if form.result:
                ...
                if self.db_manager:
                    if update_mode == 'always':
                        ...
                        save_to_index = messagebox.askyesno(
                            "Добавление в индекс",
                            f"Занести или нет в базу данных индекса всю информацию по данному файлу?\n(УИН: {final_uin})"
                        )
                ...
                if save_to_index and self.db_manager:
                    ...
                    print(f"УИН {final_uin} добавлен в центральный индекс.")
                ...
                try:
                    if pdf_path != new_name:
                        ...
                        print(f"Переименован: {pdf_path.name} -> {new_name.name}")
                except Exception as e:
                    print(f"Ошибка переименования: {e}")
            else:
                print(f"Пропущено пользователем: {pdf_path.name}")

        # --- Phase 2: Generate Statement Documents (.docx, .pdf) ---
        if messagebox.askyesno("Генерация документов", "Папка обработана. Сформировать заявление в суд (.docx и .pdf)?"):
            try:
                import generate_docs
                generate_docs.generate_for_folder(folder, self.config)
            except Exception as e:
                print(f"Ошибка при запуске генерации документов: {e}")

        messagebox.showinfo("Завершено", f"Обработка папки {folder.name} завершена.")
